...
public class Collectors extends Controller {
	...
	public static void uploadImages(Upload[] files) {
		...
		for (Upload u : files) {
			try {
				...
				args.put("id", fa.id);
				jfi.addSuccessFile(fa.name, fa.attachment.length(),
						Router.getFullUrl("Collectors.showImage", args),
						Router.getFullUrl("Collectors.showImage", args), "",
						"DELETE");

				Http.Response.current().setHeader("Content-Length",
						String.valueOf(files[0].asFile().length()));
			} catch (Throwable e) {
				Logger.error(e, "uploadImages");
				jfi.addErrorFile(u.getFileName(), u.getSize(), e.getMessage());
			}
			// jfi.
			Logger.info("file: %s", u.getFileName() + ":" + u.getContentType()
					+ ":" + u.getFileName());
		}
	...
	public static void showImage(Long id) {
		...
		if (fa == null || fa.attachment == null)
			renderBinary(VirtualFile
					.fromRelativePath("/public/img/general-noImage.jpg")
					.getRealFile());
	 ...
	 *            the collection id
	 */
	public static void remoteItemFromCollection(Long itemId,
			Long CollectionId) {
		render();
	}
	 ...
	 *            the collection id
	 */
	public static void removeCategoryToCollection(Long categoryId,
			Long CollectionId) {
